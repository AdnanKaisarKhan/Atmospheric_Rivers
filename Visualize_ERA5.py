...
file_path = 'D:/Nepal/ERA5/ERA5_26_Sep_2024.nc'
...
if data_ret == 'hour':
    ...
    # Plot the data
    levels = np.linspace(1, 15, 10)
    precip_plot = ax.contourf(lon, lat, tp_masked, cmap='viridis_r', transform=ccrs.PlateCarree(), levels=levels, extend='max')
    ...
    plt.title(f'Total Precipitation (ERA5) (Time: {time_converted[time_index].strftime("%Y-%m-%d %H:%M:%S")})')
    
    plt.savefig('D:/Nepal/ERA5/D:/Nepal/ERA5/Plot_2014_09_06.png', dpi = 600)
